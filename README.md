...
Это готовый self-hosted прокси, который ставится на любой VPS в Европе
(или в любой стране, где OpenRouter не заблокирован) и даёт твоим
приложениям доступ к LLM через привычный **OpenAI-совместимый API**.

**Главная проблема, которую решает:** OpenRouter.ai заблокирован по GeoIP
для российских IP-адресов (как и большинство LLM-провайдеров напрямую —
OpenAI, Anthropic, Google). Если ты в России и пытаешься ходить на
openrouter.ai из своего браузера, скрипта или Cursor — получишь 403.

**Решение:** поднять VPS в Европе, накатить этот стек, и ходить уже
на свой собственный домен. Внутри nginx принимает HTTPS, проксирует
в LiteLLM, тот подменяет твой токен на реальный ключ OpenRouter и шлёт
запрос от IP европейского сервера. Клиент (ты) получает ответ как
от обычного OpenAI API.

**Кому это нужно:**
- Разработчикам из РФ/РБ/etc, которым нужен Claude/GPT/Gemini/Llama/DeepSeek/etc
  в своих скриптах, агентах, IDE-плагинах (Cursor, Continue, Cline)
- Тем, кто хочет **единый токен** для всех моделей (вместо ключей OpenAI
  + Anthropic + Google + OpenRouter по отдельности)
- Тем, кому нужны **виртуальные ключи с лимитами** — выдаёшь коллегам
  или агентам отдельный `sk-...` с лимитом бюджета и доступом только
  к определённым моделям
- Тем, кому нужен **spend-мониторинг** — сколько потрачено, на чём,
  кем, по каждому ключу
...
Стек собран из четырёх контейнеров в `docker-compose.yml`:

| Сервис | Образ | Что делает |
|---|---|---|
| **nginx** | `nginx:alpine` | Принимает HTTPS на 443, отдаёт LE-сертификат, проксирует `/` → `litellm:4000`. HTTP (80) редиректит на HTTPS, кроме `/.well-known/acme-challenge/` (для выпуска сертификата). |
| **litellm** | `ghcr.io/berriai/litellm-database:main-stable` | Ядро прокси. OpenAI-совместимый API на порту 4000, читает `config.yaml` + Postgres, балансирует запросы между моделями, считает spend, отдаёт Admin UI на `/ui`. |
| **postgres** | `postgres:16-alpine` | Хранит виртуальные ключи, spend-логи, список моделей (если добавлять через UI), команды. Без него Admin UI и виртуальные ключи **не работают** (SQLite LiteLLM официально не поддерживает). |
| **certbot** | `certbot/certbot` | Запускает `certbot renew` каждые 12 часов — продлевает LE-сертификат за 30 дней до истечения автоматически. |

Опционально на хосте: 2 ГБ **swap**-файл (нужен если RAM < 2 ГБ — иначе
LiteLLM падает в OOM).

**Файлы:**

| Файл | Зачем |
|---|---|
| `docker-compose.yml` | Описание всех 4 контейнеров + сетей + volumes |
| `litellm/config.yaml` | Список моделей, цены, provider-routing, fallback'и |
| `nginx/conf.d/app.conf` | Виртуальный хост nginx с TLS и проксированием |
| `init-letsencrypt.sh` | Первый запуск: поднимает временный self-signed, выпускает LE-сертификат, перезапускает стек |
| `bootstrap.sh` | Полная установка на чистом сервере одной командой (Docker, ufw, swap, клонирование, секреты) |
| `.env` | **Не в репо** — содержит `OPENROUTER_API_KEY`, `LITELLM_MASTER_KEY` и т.д. с правами 600 |
| `.env.example` | Шаблон `.env` со всеми переменными, в репо |

## Безопасность: какие ключи где живут

- `OPENROUTER_API_KEY` — настоящий ключ OpenRouter, знает только сервер.
  Хранится в `/root/openrouter-proxy/.env` (chmod 600), попадает в контейнер
  `litellm` через `env_file` и используется для авторизации на openrouter.ai.
  **Никогда не давай его клиентам** — по нему можно списать всё, что есть
  на аккаунте OpenRouter.
- `LITELLM_MASTER_KEY` — твой собственный токен, который ты раздаёшь
  клиентам/IDE/агентам. Это одновременно и пароль в Admin UI.
  Сгенерируй длинный: `openssl rand -hex 32`.
- `LITELLM_SALT_KEY` — ключ шифрования, которым LiteLLM шифрует
  провайдерские ключи в БД. Задаётся **один раз и навсегда**, иначе
  расшифровка сломается при следующем старте. Сгенерируй и сохрани
  отдельно: `openssl rand -hex 32`.
- `POSTGRES_PASSWORD` — пароль к БД, нужен только для прямого
  подключения к Postgres (если захочешь делать SQL-запросы). Сгенерируй:
  `openssl rand -hex 24`.
- Виртуальные ключи (`sk-...`) — создаются в Admin UI (`/ui` →
  Keys → Create Key), позволяют выдать коллегам **отдельный токен**
  с лимитом бюджета в долларах, rpm/tpm, доступом только к части моделей.
  Клиент шлёт `Authorization: Bearer sk-...` — и LiteLLM считает
  трафик персонально.
